{
  pkgs ? import <nixpkgs> {},
  internalDomain ? "c.potatofrom.space",
  externalDomain ? "zero.cluster.potatofrom.space",
  serviceClusterIp ? "192.168.1.5"
}:
let
  runWithCFSSL = name: cmd:
    builtins.fromJSON (builtins.readFile (
      # Must use string replace because otherwise names like system:node:otto
      # will not validate
      pkgs.runCommand "${builtins.replaceStrings [":"] ["x"] name}-cfss.json" {
  ...
  createClientCertKey = { ca, cn, groups ? [], size ? 2048, name ? (builtins.replaceStrings [":"] [""] cn) }:
    noCSR (
  ...
  etcd = createServingCertKey {
    inherit ca;
    cn = "etcd";
    hosts = ["etcd.${externalDomain}" serviceClusterIp];
  };
  ...
  apiserver-client = {
    kubelet-otto = createClientCertKey {
      inherit ca;
      cn = "system:node:otto";
      groups = ["system:nodes"];
    };
    kubelet-rem = createClientCertKey {
      inherit ca;
      cn = "system:node:rem";
      groups = ["system:nodes"];
    };
...
in {
  rem = pkgs.buildEnv {
    ...
    paths = [
      ...
      (writeCFSSL kubelet)
      (writeCFSSL apiserver-client.kubelet-rem)
  ...
  otto = pkgs.buildEnv {
    ...
    paths = [
      (writeCFSSL (noKey ca))
      (writeCFSSL kubelet)
      (writeCFSSL apiserver-client.kubelet-otto)
      (writeCFSSL apiserver-client.kube-proxy)
      (writeCFSSL etcd-client)
    ];
  };
  ...
  admin = pkgs.buildEnv {
    ...
    paths = [
      (writeCFSSL ca)
      (writeCFSSL apiserver-client.admin)
    ];
  };
}
